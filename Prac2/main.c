...
// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  //LL_GPIO_IsInputPinSet() is defined LL in HAL library
	  if (LL_GPIO_IsInputPinSet(GPIOA, LL_GPIO_PIN_0) == BUTTON_PRESSED) {
		  if (buttonFlag == BUTTON_NOT_PRESSED) {  // Check button press
			  if(htim16.Instance->ARR == sec) {
				  htim16.Instance->ARR  = halfsec;  // Delay to half a second
			  }
			  else
			  	  {
				  	  htim16.Instance->ARR = sec;  // Delay back to a second
			  	  }
			  buttonFlag = BUTTON_PRESSED;  // Update the flag
		  	 }
	  	  }
	     else
	     {
		  buttonFlag = BUTTON_NOT_PRESSED;  // Reset the flag when the button is not pressed

		  HAL_Delay(50);  // delay for debouncing
	     }
  	 }
...
void TIM16_IRQHandler(void)
{
	...
	ledOutput = read_from_address(currentIndex); // Read data from EEPROM address pointed by currentIndex

	// Check if the read data doesn't match the expected pattern
	if (ledOutput != patterns[currentIndex]) {
	    ledOutput = 0x01; //Indicate THE ERROR
	}

	// Check if currentIndex is within the bounds of THE array
	if (currentIndex < (sizeof(patterns) - 1)) {
	    currentIndex++;
	} else {
